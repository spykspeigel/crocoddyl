...
void ContactModel3DTpl<Scalar>::calcDiff(const boost::shared_ptr<ContactDataAbstract>& data,
  ...
  if (gains_[1] != 0.) {
    d->da0_dx.leftCols(nv_l).noalias() += gains_[1] * d->fXj.template topRows<3>() * d->v_partial_dq.leftCols(nv_l);
    d->da0_dx.middleCols(nv_l,nv_l).noalias() += gains_[1] * d->fXj.template topRows<3>() * d->a_partial_da.leftCols(nv_l);
  }
}
